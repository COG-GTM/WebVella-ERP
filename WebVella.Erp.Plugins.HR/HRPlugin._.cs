...
namespace WebVella.Erp.Plugins.HR
{
	...
	{
		// INIT version MUST be strictly less than the first patch date below,
		// otherwise (currentPluginSettings.Version < patchVersion) is false and
		// the patch never runs (no entities created).
		private const int WEBVELLA_HR_INIT_VERSION = 20250100;
			...
			using (SecurityContext.OpenSystemScope())
			{
				...
				using (var connection = DbContext.Current.CreateConnection())
				{
					try
					{
						...
						var currentPluginSettings = new PluginSettings() { Version = WEBVELLA_HR_INIT_VERSION };
						string jsonData = GetPluginData();
						if (!string.IsNullOrWhiteSpace(jsonData))
							currentPluginSettings = JsonConvert.DeserializeObject<PluginSettings>(jsonData);

						//Patch 20250101 - initial HR metadata
						{
